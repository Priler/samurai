...
async def on_bu(message: types.Message):
  await message.reply(random.choice(["Гадости тебе, а не сладости пон :3 <i>пхпхпп</i>", f"На тебе 🍬 ({random.randrange(1, 100)}шт.)", "Держи шоколадку 🍫", "Печеньку, сэр 🍪", "Вотб тебе пирог 🥧", "Вотб 🍭", "Сладкое вредно для зубов!", "Хватит жрать сладости пон :3", "Будешь много кушатб сладостей, код не будет компилиться пхпхпх :3"]))
