...
async def cmd_ping_bot(message: types.Message):
    ...
    if user.is_chat_admin():
        ...
        reply += "<b>CPU:</b> <i>" + str(psutil.cpu_count()) + " cores (" + str(
            psutil.cpu_freq().max) + "MHz) with " + str(psutil.cpu_percent()) + "% current usage</i>\n"
        reply += "<b>RAM:</b> <i>" + \
            str(ram.used >> 20) + "mb / " + str(ram.total >> 20) + "mb</i>\n"

        reply += "\n<b>Bot version:</b> <i>" + \
            str(config.bot.version) + " codename «<b>" + \
...
async def cmd_profanity_check(message: types.Message):
    ...
    if user.is_chat_admin():
        ...
        line_info_ru = censor_ru.clean_line(
            utils.remove_prefix(message.text, "!prof "))
        line_info_en = censor_en.clean_line(
            utils.remove_prefix(message.text, "!prof "))
        ...
        if _del:
            ...
            if _word:
                ...
                log_msg += utils.remove_prefix(message.text, "!prof ").replace(
                    _word, '<u><b>'+_word+'</b></u>')
                log_msg += "\nПаттерн: " + _pat
